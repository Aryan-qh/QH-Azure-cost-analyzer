...
// Set yesterday's date as default for anomaly detection
...
function createSubscriptionCard(name, data) {
    ...
    if (!data.results || data.results.length === 0) {
        ...
        data.results.forEach(result => {
            /* 
             * CHANGE #1: Updated percentage change color logic
             * 
             * Previous Logic:
             * - Used simple >= 0 check, making 0% show as red
             * 
             * New Logic:
             * - percent_change > 0: red (cost increase - bad)
             * - percent_change < 0: green (cost decrease - good)  
             * - percent_change === 0 or very close to 0: green (neutral - no change is good)
             * 
             * This provides better visual feedback where:
             * - Green = good (no change or decrease)
             * - Red = bad (increase in costs)
             * 
             * Note: We use a small epsilon (0.001) to catch floating point rounding
             */
            let changeClass;
            const epsilon = 0.001; // Handle floating point precision
            
            if (Math.abs(result.percent_change) < epsilon) {
                // Treat very small changes (effectively 0%) as neutral/green
                changeClass = 'change-negative';  // Green for 0%
            } else if (result.percent_change > 0) {
                changeClass = 'change-positive';  // Red for increases
            } else {
                changeClass = 'change-negative';  // Green for decreases
            }
    ...
    return `
        <div class="card subscription-card ${hasAnomalies ? 'has-anomaly' : ''}">
            ...
            <div style="color: var(--text-secondary); margin-bottom: 1rem;">
                Comparing <strong>${formatDate(data.target_date)}</strong> against ${daysInAverage}-day rolling average
                <br>
                <small>Average period: ${formatDate(data.start_date)} to ${formatDate(data.end_date)} (${daysInAverage} days)</small>
            </div>
            <table>
                <thead>
                    <tr>
